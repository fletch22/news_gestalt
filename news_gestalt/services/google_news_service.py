...
from news_gestalt.DateRange import DateRange
from news_gestalt.config import constants
from news_gestalt.services import file_services, ticker_service
from news_gestalt.utils import date_utils
from news_gestalt.utils.PrinterThread import PrinterThread
from news_gestalt.utils.Stopwatch import Stopwatch
...
def query(news_topic: str, output_path: Path, date_range: DateRange, max_results: int = 12):
    ...
    try:
        ...
        while True:
            ...
            elapsed_secs = stopwatch.end(print_elapsed=False)
            pt.print(f"query: {news_topic}; {from_date_str}-{to_date_str} returned results: {found_content}: elapsed_secs: {elapsed_secs}")
...
def roll_through_the_days(date_range: DateRange, output_path: Path):
    ticker_tuples = ticker_service.get_ticker_searchable_tuples()
